...
match-pageout() {
    ...
    round_number=0
    while ! ( memtierd-command "stats -t move_pages -f csv | awk -F, \"{print \\\$6\\\$7}\""; grep ${pageout_regexp} <<< $COMMAND_OUTPUT); do
